        ...
        VariablePlugin = RTE("variable-plugin", async (rte) => {
            ...
            if(config.content_type && config.field && config.group_title){
                ...
                Object.values(files).forEach(
                    ...
                    invalidConfig.errorHeading = typeof err === 'string' ? 'Invalid Content Type UID' : 'Invalid Field UID'
                    invalidConfig.errorMessage = typeof err === 'string' ? 'The content type provided in app configuration is either invalid or does not exist. Please verify the app config settings in Marketplace and try again.' : 'The field provided in app configuration is either invalid or does not exist. Please verify the app config settings in Marketplace and try again.'
                }
            }
        return ({
